...
<section class="wl">
	<div class="wl-head">
		...
		<div class="wl-sort">
			Sort By:
			<button
				on:click={() => updateSort("title")}
				class="btn-link"
				class:active={sortBy === "title"}>Title</button
			>
			<span class="wl-sort-arrow" class:rotated={sortOrder === "asc"}
				>&uarr;</span
			>
			<button
				on:click={() => updateSort("price")}
				class="btn-link"
	...
	{#each gameList as item}
		<section class="wl-row">
			...
			<button class="btn" on:click={() => removeGame(item.url)}
				>Remove</button
			>
		</section>
	{/each}
